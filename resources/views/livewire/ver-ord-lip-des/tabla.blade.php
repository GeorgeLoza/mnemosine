                ...
                @foreach ($verificaciones as $verificacion)
                    <tr
                        ...
                        <td class="px-2 py-1.5">
                            {{ $verificacion->ordLimDes->sector->nombre }}
                        </td>
                        <td class="px-2 py-1.5">
                            {{ $verificacion->ordLimDes->nombre }}
                        </td>
                        <td class="px-2 py-1.5 text-center text-white {{$verificacion->limpieza ? 'bg-green-500' : 'bg-red-500'}}">
                            {{$verificacion->limpieza ? 'Si' : 'No'}}
                        </td>
                        <td class="px-2 py-1.5 text-center text-white {{$verificacion->desinfeccion ? 'bg-green-500' : 'bg-red-500'}}">
                            {{$verificacion->desinfeccion ? 'Si' : 'No'}}
                        </td>
                        <td class="px-2 py-1.5 text-center text-white {{$verificacion->profunda ? 'bg-green-500' : 'bg-red-500'}}">
                            {{$verificacion->profunda ? 'Si' : 'No'}}
                        </td>
                        <td class="px-2 py-1.5">
