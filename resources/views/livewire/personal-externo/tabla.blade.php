            ...
            <tbody>
                @foreach ($externos as $externo)
                    <tr
                        ...
                        <td
                            class="px-2 py-1.5 text-center text-white {{ $externo->uniforme ? 'bg-green-500' : 'bg-red-500' }}">
                            {{ $externo->uniforme ? 'Si' : 'No' }}
                        </td>
                        <td
                            class="px-2 py-1.5 text-center text-white {{ $externo->higiene ? 'bg-green-500' : 'bg-red-500' }}">
                            {{ $externo->higiene ? 'Si' : 'No' }}
                        </td>
                        <td
                            class="px-2 py-1.5 text-center text-white {{ $externo->salud ? 'bg-green-500' : 'bg-red-500' }}">
                            {{ $externo->salud ? 'Si' : 'No' }}
                        </td>
                        <td
                            class="px-2 py-1.5 text-center text-white {{ $externo->objetos_extranos ? 'bg-green-500' : 'bg-red-500' }}">
                            {{ $externo->objetos_extranos ? 'Si' : 'No' }}
                        </td>
                        <td class="px-2 py-1.5">
                        ...
                        <td class="px-2 py-1.5">
                            @if (auth()->user()->rol === 'Admi' ||
                                    (auth()->user()->rol === 'Supervisor' && now()->diffInMinutes($externo->created_at) <= 60))
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"
                                    class="w-4 h-4 fill-red-600" wire:click="delete({{ $externo->id }})"
