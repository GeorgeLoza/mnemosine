<div class=" p-2 bg-white border border-gray-200 rounded-lg shadow sm:p-4 md:p-6 dark:bg-gray-800 dark:border-gray-700">
    <form wire:submit.prevent="submit" novalidate>
        ...
        <div class="flex items-center p-2">
            <label for="cantidad_producto_enviado" class="w-3/6 block text-sm font-medium text-gray-900 dark:text-white">cantidad Producto Enviado:
            </label>
            <input type="number" wire:model="cantidad_producto_enviado" name="cantidad_producto_enviado" id="cantidad_producto_enviado"
                class="w-3/6 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block  p-1 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white"
                placeholder="Cantidad enviado" />
            @error('cantidad_producto_enviado')
                <p class="mt-2 text-sm text-red-600 dark:text-red-500"> {{ $message }}</p>
            @enderror
        </div>

        <div class="flex items-center p-2">
            <label for="cantidad_producto_recepcionado" class="w-3/6 block text-sm font-medium text-gray-900 dark:text-white">cantidad Producto recepcionado
            </label>
            <input type="number" wire:model="cantidad_producto_recepcionado" name="cantidad_producto_recepcionado" id="cantidad_producto_recepcionado"
                class="w-3/6 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block  p-1 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white"
                placeholder="Cantidad Recepcionada" />
            @error('cantidad_producto_recepcionado')
                <p class="mt-2 text-sm text-red-600 dark:text-red-500"> {{ $message }}</p>
            @enderror
        </div>

        <div class="flex items-center p-2">
            <label for="temperatura_vaciado" class="w-3/6 block text-sm font-medium text-gray-900 dark:text-white">temperatura_vaciado
            </label>
            <input type="number" wire:model="temperatura_vaciado" name="temperatura_vaciado" id="temperatura_vaciado"
                class="w-3/6 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block  p-1 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white"
                placeholder="temperatura vaciado" />
            @error('temperatura_vaciado')
                <p class="mt-2 text-sm text-red-600 dark:text-red-500"> {{ $message }}</p>
            @enderror
        </div>
