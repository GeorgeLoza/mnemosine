<div class=" p-2 bg-white border border-gray-200 rounded-lg shadow sm:p-4 md:p-6 dark:bg-gray-800 dark:border-gray-700">
    <form wire:submit.prevent="submit" novalidate>
        @csrf
        <h5 class="text-xl font-medium text-gray-900 dark:text-white">Corte</h5>
        ...
        <div class="flex items-center p-2">
            <label for="temperatura_corte" class="w-3/6 block text-sm font-medium text-gray-900 dark:text-white">Temperatura de corte:
            </label>
            <input type="number" wire:model="temperatura_corte" name="temperatura_corte" id="temperatura_corte"
                class="w-3/6 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block  p-1 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white"
                placeholder="Temperatura de corte" />
            @error('temperatura_corte')
                <p class="mt-2 text-sm text-red-600 dark:text-red-500"> {{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="flex items-center p-2">
            <div class="flex items-center">
                <input id="calibracion" type="checkbox" wire:model="calibracion"
                    class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                <label for="calibracion" class="text-sm font-medium text-gray-900 dark:text-white">Calibracion del Cortador</label>
            </div>
        </div>
