...
<head>
    ...
    <title>PLP - PRO - 601 - REG - 00 Registro de Curacion y Dotacion</title>
    <style>
        @page {
            margin-top: 1cm;
            margin-bottom: 0cm;
...
<header>
    <table class="head" style="border: 1px solid black;font-size: 0.8rem; margin-bottom: 0.6rem">
        <tr>
            ...
            <th style="width: 25%; font-size: 0.8rem">PLP - PRO - 601 - REG - 04 <br> Versión 003 <br>
                <div class="page-number"></div>
            </th>
        </tr>
        <tr>
            <td colspan="3" style="text-align: center; padding: 0.6rem;  font-weight:bold;">CURACIONES LEVES /
                DOTACION DE ESPARADRAPO </td>
        </tr>
    </table>
</header>
            ...
            <table class="table-pdf">
                ...
                <thead>
                    <tr>
                        ...
                        <th>Esparadrapo</th>
                        <th>Guante</th>
                        <th>Verificaion Inicio</th>
                        <th>Verificaion Fin</th>
                        <th>Observaciones</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($datos as $dato)
                        <tr>
                            <td>{{ $dato->tiempo }}</td>
                            <td>{{ $dato->trabajador->name }} {{ $dato->trabajador->lastname }}</td>
                            <td>{{ $dato->detalle }}</td>
                            <td> {{ $dato->esparatrapo ? 'Si' : 'No' }}</td>
                            <td> {{ $dato->guante ? 'Si' : 'No' }}</td>
                            <td>{{ $dato->responsableInicio->name }} {{ $dato->responsableInicio->lastname }}</td>
                            <td>
                                @if ($dato->responsable_fin)
                                    {{ $dato->responsableFin->name }} {{ $dato->responsableFin->lastname }}
                                @endif
                            </td>
